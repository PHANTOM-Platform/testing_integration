...
echo -e "\n\n reject_exec: exec_id=\"${1}\"";
#	curl -XGET 'localhost:9400/manager_db/executions_status/AWiVZuGkbJ9V_FEcjRGS?pretty="true"'
	curl -s -H "Authorization: OAuth ${mytoken}" -H "Content-Type: text/plain" -XPOST http://${server}:${execmanager_port}/reject_exec?exec_id=${1}\&reason="something";
